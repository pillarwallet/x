...
describe('<TokenListItem />', () => {
    ...
    it('renders TokenListItem correctly with all the given arguments', () => {
        render(<TokenListItem tokenName={tokenName} tokenSymbol={tokenSymbol} chainName={chainName} onClick={onClickMock} tokenLogo={tokenLogo} />);
    ...
    it('renders TokenListItem with default logo when no tokenLogo is provided', () => {
        render(<TokenListItem tokenName={tokenName} tokenSymbol={tokenSymbol} chainName={chainName} onClick={onClickMock} />);
    
        const defaultLogo = screen.getByAltText('token-logo');
        expect(defaultLogo).toHaveAttribute('src', expect.stringContaining('logo-unknown.png'));
    });
    
    it('calls onClick when TokenListItem is clicked', () => {
        render(<TokenListItem tokenName={tokenName} tokenSymbol={tokenSymbol} chainName={chainName} onClick={onClickMock} tokenLogo={tokenLogo} />);
        
        const listItem = screen.getByText(tokenName).closest('div') as HTMLDivElement;
