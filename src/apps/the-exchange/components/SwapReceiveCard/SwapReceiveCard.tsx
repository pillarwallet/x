...
const SwapReceiveCard = ({ position, initialPosition, onClick }: SwapReceiveCardProps) => {
    ...
    return (
        <div onClick={isClickable ? onClick : undefined} className={`flex flex-col w-full cursor-pointer h-[200px] justify-between rounded-lg p-4 desktop:h-[230px] desktop:max-w-[306px] ${initialPosition === CardPosition.SWAP ? 'bg-green' : 'bg-purple'}`}>
            <SelectToken
