...
describe('<TileContainer />', () => {
    ...
    it('renders the single child correctly', () => {
      const tree = renderer.create(<TileContainer>{singleChild}</TileContainer>).toJSON() as ReactTestRendererJSON;
      const singleDivChild = tree.children?.find(child =>
        typeof child === 'object' && child.type === 'div' && child.children?.includes('Div as children')
    ...
    it('renders multiple children correctly', () => {
      const tree = renderer.create(<TileContainer>{multipleChildren}</TileContainer>).toJSON() as ReactTestRendererJSON;
      const multipleDivsChild = tree.children?.filter(child =>
        typeof child === 'object' && child.type === 'div'
      ) as ReactTestRendererJSON[];
    ...
    it('applies the custom class', () => {
      ...
      const tree = renderer.create(<TileContainer className={customClass}>{singleChild}</TileContainer>).toJSON() as ReactTestRendererJSON;
      expect(tree.props.className).toContain('flex');
      expect(tree.props.className).toContain('bg-container_grey');
      expect(tree.props.className).toContain('rounded-2xl');
