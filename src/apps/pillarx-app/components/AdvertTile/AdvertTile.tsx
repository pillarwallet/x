...
const AdvertTile = ({ data, isDataLoading }: AdvertTileProps) => {
    ...
    <Native placement={slugData.slug}>
        <TileContainer id='advert-tile' className='mobile:bg-[#1F1D23] xs:flex-col'>
            <div className='flex flex-col p-4 justify-between desktop:basis-1/2 tablet:basis-1/2 mobile:basis-1/2 xs:basis-full xs:order-2'>
                <div className='flex items-center gap-5 mb-2'>
                    <img data-ref="icon" className="h-10 w-10 rounded-full"/>
                    <div className="flex flex-col">
                        <p className='text-xs text-white'>
                            @<span data-ref="advertiser"></span>
                        </p>
                        <p className='text-xs text-purple_light' data-ref="displayUrl"></p>
                    </div>
                </div>
                <div data-ref="headline" className="desktop:text-xl tablet:text-xl mobile:text-base mb-2"></div>
                <div data-ref="body" className="text-sm text-purple_light mb-2"></div>
                <a data-ref="ctaLink" href="/" target="_blank" rel="noreferrer">
                    <p data-ref="ctaText" className="rounded-md text-base bg-purple_medium px-4 py-2 w-fit cursor-pointer"></p>
                </a>
            </div>
            <div className='flex items-center justify-end desktop:basis-1/2 tablet:basis-1/2 mobile:basis-1/2 xs:basis-full xs:order-1'>
                <a data-ref="ctaLink" href="/" target="_blank" rel="noreferrer" className='w-full'>
                    <div data-ref="mediaContent" className='flex h-auto rounded-r-2xl mobile:rounded-2xl xs:rounded-2xl overflow-hidden cursor-pointer xs:w-full'></div>
                </a>
            </div>
        </TileContainer>
    </Native >
