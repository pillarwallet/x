...
const EditorialTag = ({ label, icon, color }: EditorialTagProps) => {
    ...
    return (
        <div id='editorial-tag' className="flex relative rounded py-1 px-2 w-fit mr-2">
            <div
            ...
            <div className="flex relative items-center gap-1" style={{ zIndex: 10 }}>
                {IconTag && <IconTag data-testid="icon-tag" style={{ width: 16, height: 16, color: tagColor }} />}
                {label && <p className="text-xs" style={{ color: tagColor }}>{label}</p>}
            </div>
        </div>
    );
};
