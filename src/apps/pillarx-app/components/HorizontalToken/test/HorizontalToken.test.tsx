...
describe('<HorizontalToken />', () => {
    ...
    it('renders the logo with the correct src', () => {
        ...
        const divElement = tree.children?.find(child => 
            typeof child === 'object') as ReactTestRendererJSON;

        const imgProp = divElement.children?.find(child => 
            typeof child === 'object' && child.type === 'img') as ReactTestRendererJSON;
    ...
    it('renders the token symbol and name correctly', () => {
        ...
        const divElementSecond = divElement.children?.find(child => 
            typeof child === 'object' && child.type === 'div') as ReactTestRendererJSON;

        const tokenSymbolProp = divElementSecond.children?.find(child => 
            typeof child === 'object' && child.type === 'Body') as ReactTestRendererJSON;
        
        const tokenNameProp = divElementSecond.children?.find(child => 
            typeof child === 'object' && child.type === 'BodySmall') as ReactTestRendererJSON;
    ...
    it('renders the token value correctly', () => {
        ...
        const divElement = tree.children?.filter(child => 
            typeof child === 'object' && child.type === 'div') as ReactTestRendererJSON[];

        const tokenValueProp = divElement[1].children?.find(child => 
            typeof child === 'object' && child.type === 'Body') as ReactTestRendererJSON;
