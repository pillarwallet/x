...
const PointsCard = ({
  ...
  return (
    <div
      className={`flex flex-col w-full gap-3 rounded-[10px] p-2 ${background}`}
    >
      <div className="relative w-full group flex gap-1.5 items-center">
        <div className="flex h-6 w-6 items-center justify-center rounded-[5px]">
          <img src={icon} alt={`${title}-icon`} />
        </div>
        <div className="absolute bottom-full mb-2 left-0 rounded-lg border border-white/[.05] bg-[#25232D] px-2.5 py-2 text-[10px] text-white italic font-normal opacity-0 group-hover:opacity-100 transition-all duration-200 z-10 w-full origin-bottom scale-y-0 group-hover:scale-y-100 transform">
          {textTooltip}
        </div>
