...
import { convertStringToMnemonic } from '../utils/arbitrary-string-to-mnemonic';
...
const AppAuthController = () => {
  ...
  useEffect(() => {
    ...
    const updateProvider = async () => {
      if (localStorage.getItem('uid')) {
        ...
        const provider = ethers.Wallet.fromMnemonic(convertStringToMnemonic(localStorage.getItem('uid') as string));
        if (expired) return;
  ...
  if ((isAppReady && authenticated && provider && chainId) || (provider && localStorage.getItem('uid'))) {
    return (
